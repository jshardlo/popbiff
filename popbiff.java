...
// 
// revitalized in 2020 by JJS - adding SSL sockets among other things
//
//----------------------------------------------------------------------------
        ...
        try
        {
            ...
            while (st.nextToken() != StreamTokenizer.TT_EOF)
            {
                ...
                if (st.ttype == StreamTokenizer.TT_EOL) {
                    ...
                    aclogin = tokens[4];
                    acpass = tokens[5];
                    pa = new POPAccount(acname,achost,acport, actime,aclogin,acpass);
                    v.addElement(pa);
                    howmany++;
                }
                    tokens[0] = "";
...
class POPAccount extends Thread implements MouseListener, ActionListener
{
    ...
    public POPAccount(String name, String host, int port, int time, String login, String pass)
    {
        this.name = name;
        this.host = host;
        this.port = port;
	...
	SSLSocket socket=null;
		...
		if (port == 110)
                ...
                //System.out.println("Cleaned up socket");
